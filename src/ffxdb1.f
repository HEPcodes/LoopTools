...
*		IR divergent case
	  if(m2 .eq. 0D0 .and. p .eq. m1) then
	    if(.not. initir) then
	      initir = .TRUE.
	      print *, "ffxdb1: IR divergent B1', ",
     +          "using cutoff ", delta
	    endif
	    cdb1 = .5D0*(3D0 + log(delta/p))/p
	    return
	  endif
	  ...
	  else if(abs(xp) .gt. 10D0) then
	    cdb1 = DBLE( (2D0/3D0 +
     +       (2D0 - 3D0*xp)*ffpvf(3, xp, yp))/x1**2 )/p
	  else if(abs(yp) .gt. precx) then
	    cdb1 = DBLE( (3D0/2D0 +
     +       (2D0 - 3D0*xp)*ffpvf(1, xp, yp)) )/p
	  else
	    call fferr(101, ier)
	  endif
...
*		zero momentum case
	  ...
	  if(abs(x2) .lt. 10D0) then
	    cdb1 = -(1D0/3D0 + ffypvf(2, xm, ym))/(m1 - m2)
	  else
	    cdb1 = -(1D0/3D0 + ffypvf(3, xm, ym))/m1
	  endif
...
*
	...
	if(abs(p) .gt. precx*(m1 + m2)) then
	  ...
	  if(abs(xp - xm) .gt. sqrt(precx)*abs(xp + xm)) then
	    ...
	    cdb11 = DBLE( (-3D0/4D0 +
     +       (4D0*xp - 3D0)*ffpvf(4, xp, yp))/x1**2 )/p
	  else if(abs(yp) .gt. precx) then
	    cdb11 = DBLE( (-4D0/3D0 +
     +       (4D0*xp - 3D0)*ffpvf(2, xp, yp))/p )
	  else
	    call fferr(102, ier)
	  endif

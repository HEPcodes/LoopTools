...
\usepackage{a4wide,array,epsfig,amsmath,axodraw,makeidx,calc,alltt}
...
\Huge \LT~2.6~~~User's Guide}}

\vspace*{1ex}

\hfill\hbox{December 14, 2010~~~~~Thomas Hahn}
...
\cite{De93}.  \LT\ offers three interfaces, Fortran, C++, and \mma, so 
...
To compile the package, a Fortran 77 compiler and the GNU C compiler 
(\Code{gcc}) are required.

\LT\ comes in a compressed tar archive \Code{LoopTools-2.6.tar.gz}. 
Execute the following commands to unpack and compile the package.
\bbox
\begin{verbatim}
   gunzip -c LoopTools-2.6.tar.gz | tar xvf -
   cd LoopTools-2.6
   ./configure
   make
   make install
...
the \Code{LoopTools/\$HOSTTYPE} directory:
\begin{tabbing}
...
\rlap{\Code{LoopTools/}}\hspace{.3\linewidth} \=
	the \LT\ directory \\
\Code{\greyed{LoopTools/}\$HOSTTYPE/} \>
...
\indextt{HOSTTYPE}%
\Code{HOSTTYPE} is an environment variable that is set by most shells to
a string identifying the system, \eg \Code{alpha}, \Code{hp9000s700}, or
\Code{i586-linux}.  Its purpose as a directory name is to separate the
binaries for different platforms.  To see what its value is on your system,
type the following command at the shell prompt:
\begin{verbatim}
   tcsh -c "echo $HOSTTYPE"
\end{verbatim}
...
\Code{LoopTools/\$HOSTTYPE}.  To this end, configure with \eg
\begin{verbatim}
...
\frac{(2\pi\mu)^{4 - D}}{\i\pi^2} \int\d^Dq\,
\frac{q_{\mu_1}\cdots q_{\mu_P}}
  {\bigl[q^2 - m_1^2\bigr]\,
   \bigl[(q + k_1)^2 - m_2^2\bigr] \cdots
   \bigl[(q + k_{N - 1})^2 - m_N^2\bigr]}
\end{equation}
...
\Text(72,15)[tc]{$m_2$}
\end{picture}}
= \frac{(2\pi\mu)^{4 - D}}{\i\pi^2} \int
\frac{\text{(numerator)}~\d^D q}
...
the \Code{B0i} and \Code{Bget} coefficients include the derivatives,
so there is no \Code{DB0i} or \Code{DBget}.
...
\index{Fortran}%
\index{f77 command line@\Code{f77} command line}%
...
\end{verbatim}
on the \Code{f77} command line.  As Unix linker are one-pass linkers, 
the library flags (\Code{-L...}, \Code{-l...}) must come after the 
Fortran or object files on the command line.  In a makefile, you have 
to use parentheses around the environment variables, \ie \Code{\$(LT)} 
instead of \Code{\$LT}.
...
through the C preprocessor first.  If you are using an older Fortran
compiler which does not recognize the \Code{.F} extension, use the script
\Code{F77} in the \Code{\$LT/bin} subdirectory instead of the normal
\Code{f77}.
\end{itemize}
...
\section{Using \LT\ with C++}

\index{C++}%
\index{g++ command line@\Code{g++} command line}%
...
\item In bare C there is no intrinsic type \Code{complex}, and using things
like \Code{structs} is rather annoying.  Therefore, I have made no attempt
to implement the tensor package in bare C.  Unfortunately, even in C++
the type \Code{complex} is anything but standardized.  As it stands, the
package will compile with \Code{g++} 2.7.2 and higher.  Using any non-GNU 
C++ compiler is at your own risk.  If you really must work with a 
different compiler, the most likely cause of problems is the different
naming convention for complex variables.  If so, the following compiler
switch will help:
\begin{verbatim}
   -Ddouble_complex=mycomplexname
\end{verbatim}
where \Code{mycomplexname} is your compiler's name for the complex data
type, usually something like \Code{complex}, \Code{Complex} etc.
\end{itemize}

To use the \LT\ functions in a C++ program, the file \Code{clooptools.h}
must be included.  Similar to the Fortran case, before making the first
call to any \LT\ function, \Code{ltini()} must be called and at the end
\Code{ltexi()} may be called to get a summary of errors.
...
bosonic part of the Higgs self-energy in the electroweak Standard Model.
At first sight, the C++ code looks more appealing, but remember that
\Code{double\uscore complex} is not an intrinsic data type in C++ and that
certainly compilation time and usually also the performance of the
resulting program will be worse than in Fortran.
\begin{verbatim}
#include <complex.h>
#include <fstream.h>
#include "clooptools.h"
...
static double_complex SigmaH(double k2) {

...
\Huge \LT~2.10~~~User's Guide}}

\vspace*{1ex}

\hfill\hbox{February 26, 2014~~~~~Thomas Hahn}
...
\LT\ comes in a compressed tar archive \Code{LoopTools-2.9.tar.gz}. 
...
\indextt{nocache}%
The `\Code{nocache}' versions do not make use of LoopTools' internal
caching system.  They might be useful in parallel environments since the
cached versions are not thread-safe.  They are not available in
Mathematica.
...
\eg for parallel execution.  The ``\Code{$N$put}'' subroutines are 
nevertheless not thread-safe, due to the caching mechanism.

\indextt{nocache}%
The ``\Code{$N$putnocache}'' and ``\Code{$N$0nocache}'' subroutines do 
not use the cache and can be used for parallel execution, provided the 
Fortran compiler produces reentrant code (that is, the compiler must 
not allocate local variables of a subroutine statically; this may be a 
problem for older F77 compilers).
